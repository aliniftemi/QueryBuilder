...
public class SqlInjectionAttackFilter implements IRuleFilter
{
	...
	public SqlInjectionAttackFilter(EnumDBType dbType) {
		...
		if (EnumDBType.MYSQL.equals(dbType)) {
			...
			if (EnumDBType.ORACLE.equals(dbType)) {
				this.beginningDelimiter = '"';
				this.endingDelimiter = '"';
				file = "keywords-oracle.txt";
			}
			else
				if (EnumDBType.MS_SQL.equals(dbType)) {
					this.beginningDelimiter = '[';
					this.endingDelimiter = ']';
		...
		try (InputStream inputStream = SqlInjectionAttackFilter.class.getClassLoader().getResourceAsStream(file); InputStreamReader inputStreamReader = new InputStreamReader(inputStream); BufferedReader bufferedReader = new BufferedReader(inputStreamReader)) {
			String line;
	...
	public void doFilter(JsonRule jsonRule, EnumBuilderType type) throws FilterException {
		...
		if (!jsonRule.isGroup()) {
			...
			if ((EnumDBType.MYSQL.equals(this.dbType) && field.length() > 64) || (EnumDBType.ORACLE.equals(this.dbType) && field.length() > 30) || (EnumDBType.MS_SQL.equals(this.dbType) && field.length() > 128)) {
